...
import com.hongul.filq.ui.navigation.NavItem
import kotlinx.coroutines.launch

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun BusinessCardGenerateScreen(navigator: NavHostController, viewModel: CustomizeViewModel) {
    ...
    var selectedAlignment by remember { mutableStateOf(Alignment.BottomStart) }
                    ...
                    IconButton(onClick = {
                        when {
                            currentSNS != null -> currentSNS = null // URL 입력 화면에서 뒤로가기
                                ...
                                when (ps.currentPage) {
                                    0 -> navigator.popBackStack() // 첫 페이지에서 네비게이션 뒤로
                                    1 -> scope.launch { ps.animateScrollToPage(0) } // "명함 사진 불러오기란?"에서 첫 페이지로
                                    2 -> scope.launch { ps.scrollToPage(0) }
                                    in 3..5 -> scope.launch { ps.animateScrollToPage(ps.currentPage - 1) } // 일반 뒤로가기
                                    6 -> scope.launch { ps.scrollToPage(4) } // URL 페이지에서 SNS 추가로 이동
                                    7 -> scope.launch { ps.animateScrollToPage(6) } // 7번 페이지에서 6번 페이지로 이동
                                    8 -> scope.launch { ps.animateScrollToPage(7) } // 8번 페이지에서 7번 페이지로 이동
                                    9 -> scope.launch { ps.animateScrollToPage(8) } // 9번 페이지에서 8번 페이지로 이동
                                    10 -> scope.launch { ps.animateScrollToPage(9) } // 10번 페이지에서 9번 페이지로 이동
                                    11 -> scope.launch { ps.scrollToPage(6) } // 11번 페이지에서 6번 페이지로 이동
                                    12 -> scope.launch { ps.animateScrollToPage(11) } // 12번 페이지에서 11번 페이지로 이동
                                    else -> scope.launch { ps.animateScrollToPage(0) } // 기본적으로 첫 페이지로 이동
                                }
                            }
                        }
                    }) {
        ...
        Column(
            ...
            if (currentSNS != null) {
                ...
                URLPage(
                    ...
                    onRegisterClick = { type, url ->
                        val sns = when(type) {
                            "Facebook" -> SNS.Facebook(url)
                            "X" -> SNS.X(url)
                            "Instagram" -> SNS.Instagram(url)
                            "YouTube" -> SNS.Youtube(url)
                            else -> null
                        }!!
                        val snsList = info["sns"] as List<SNS>
                        info["sns"] = snsList + sns
                        scope.launch { ps.scrollToPage(4) }
                                        ...
                                        scope.launch {
                                            viewModel.insertCard(
                                                BusinessCard(
                                                    ...
                                                    organization = info["organization"] as String,
                                                    department = info["department"] as String,
                                            ...
                                            Log.d("BusinessCardPreviewScreen", "완료 버튼 클릭됨")
                                            navigator.navigate(NavItem.Home.route) // 완료 버튼 클릭 시 첫 페이지로 이동
                                        }
                                    },
                                    onBackClick = {
                                        scope.launch {
